  ...
  const handleFilter = e => {
    ...
    if (value.length) {
      updatedData = data.filter(item => {
        const startsWith =
          item.nama ? item.nama.toLowerCase().startsWith(value.toLowerCase()) : item.username.toLowerCase().startsWith(value.toLowerCase()) ||
          ...
          <div className='user-info text-truncate ms-1'>
            <Link to={`/admin/users/detail/${row.id}`}>
              <span className='d-block fw-bold text-truncate'>{
                row.nama ? row.nama : row.username
              }</span>
            </Link>
